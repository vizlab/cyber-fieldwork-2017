...
def do_left_timestep(u0, u, vx,vy):
    ...
    u[1:-1, 1:-1] = u0[1:-1, 1:-1] + (dt * vx[1:-1, 1:-1] * (u0[2:, 1:-1] - u0[1:-1, 1:-1]) / dx) + (dt * vy[1:-1, 1:-1] * (u0[1:-1, 2:] - u0[1:-1, 1:-1]) / dy)
    u0 = u.copy()
    return u0, u
...
def do_up_timestep(u0, u, vx,vy):
    # Propagate with forward-difference in time, central-difference in space
    u[1:-1, 1:-1] = u0[1:-1, 1:-1] + (dt * vx[1:-1, 1:-1] * (u0[1:-1, 2:] - u0[1:-1, 1:-1]) / dx) + (dt * vy[1:-1, 1:-1] * (u0[2:, 1:-1] - u0[1:-1, 1:-1]) / dy)
    u0 = u.copy()
    return u0, u
...
for key, direction in enumerate(['left', 'down', 'right', 'up']):
    ...
    filePath = './front/public/convection' + str(key) + '.json'
